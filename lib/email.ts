...
const FROM = process.env.EMAIL_FROM ?? "Capilytix <onboarding@resend.dev>";
...
function shell(title: string, body: string): string {
  return `<div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;background:#FBF7F1;padding:32px 16px">
    <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:20px;padding:32px;border:1px solid #ECE3D6">
      <div style="font-size:22px;font-weight:700;color:#2A2521;margin-bottom:18px">Capilytix</div>
      ...
      <p style="font-size:12px;color:#A89F93;margin-top:24px">Capilytix — ton coach capillaire IA</p>
    </div>
  </div>`;
}
...
export function welcomeEmail() {
  return {
    subject: "Bienvenue dans ton Cycle Capilytix 🌱",
    html: shell(
